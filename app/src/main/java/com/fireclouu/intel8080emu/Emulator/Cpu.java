...
public class Cpu {
	...
	public short step() {
		...
		switch (opMemory) {	
			...
			case 0xeb:
				...
				// IMMEDIATE
			case 0x06: b = mmu.readMemory(opcode + 1); pc++; break; // MVI B, D8
			case 0x0e: c = mmu.readMemory(opcode + 1); pc++; break; // MVI C, D8
			case 0x16: d = mmu.readMemory(opcode + 1); pc++; break; // MVI D, D8
			case 0x1e: e = mmu.readMemory(opcode + 1); pc++; break; // MVI E, D8
			case 0x26: h = mmu.readMemory(opcode + 1); pc++; break; // MVI H, D8
			case 0x2e: l = mmu.readMemory(opcode + 1); pc++; break; // MVI L, D8
			case 0x36: 
				mmu.writeMemory(addr, mmu.readMemory(opcode + 1));
				pc++; 
				break; // MVI M, D8
			case 0x3e: a = mmu.readMemory(opcode + 1); pc++; break; // MVI A, D8
			...
			case 0x46: b = mmu.readMemory(addr); break; // MOV B, M
			...
			case 0x4e: c = mmu.readMemory(addr); break; // MOV C, M
			...
			case 0x56: d = mmu.readMemory(addr); break; // MOV D, M
			...
			case 0x5e: e = mmu.readMemory(addr);; break; // MOV E, M
			...
			case 0x66: h = mmu.readMemory(addr);; break; // MOV H, M
			...
			case 0x6e: l = mmu.readMemory(addr); break; // MOV L, M
			case 0x6f: l = a; break; // MOV L, A

				// MEMORY
			case 0x70: mmu.writeMemory(addr, b); break; // MOV M, B
			case 0x71: mmu.writeMemory(addr, c); break; // MOV M, C
			case 0x72: mmu.writeMemory(addr, d); break; // MOV M, D
			case 0x73: mmu.writeMemory(addr, e); break; // MOV M, E
			case 0x74: mmu.writeMemory(addr, h); break; // MOV M, H
			case 0x75: mmu.writeMemory(addr, l); break; // MOV M, L
			case 0x77: mmu.writeMemory(addr, a); break; // MOV M, A
			...
			case 0x7e: a = mmu.readMemory(addr); break; // MOV A, M
			case 0x7f: break; // MOV A, A

				///  ALU  ///

				// ADD
			case 0x80: instr_add(b, 0); break; // ADD B
			case 0x81: instr_add(c, 0); break; // ADD C
			case 0x82: instr_add(d, 0); break; // ADD D
			case 0x83: instr_add(e, 0); break; // ADD E
			case 0x84: instr_add(h, 0); break; // ADD H
			case 0x85: instr_add(l, 0); break; // ADD L
			case 0x86: instr_add(mmu.readMemory(addr), 0); break; // ADD M
			case 0x87: instr_add(a, 0); break; // ADD A

				// ADC
			case 0x88: instr_add(b, cc.cy); break; // ADC B
			case 0x89: instr_add(c, cc.cy); break; // ADC C
			case 0x8a: instr_add(d, cc.cy); break; // ADC D
			case 0x8b: instr_add(e, cc.cy); break; // ADC E
			case 0x8c: instr_add(h, cc.cy); break; // ADC H
			case 0x8d: instr_add(l, cc.cy); break; // ADC L
			case 0x8e: instr_add(mmu.readMemory(addr), cc.cy); break; // ADC M
			case 0x8f: instr_add(a, cc.cy); break; // ADC A

				// SUB
			case 0x90: instr_sub(b, 0); break; // SUB B
			case 0x91: instr_sub(c, 0); break; // SUB C
			case 0x92: instr_sub(d, 0); break; // SUB D
			case 0x93: instr_sub(e, 0); break; // SUB E
			case 0x94: instr_sub(h, 0); break; // SUB H
			case 0x95: instr_sub(l, 0); break; // SUB L
			case 0x96: instr_sub(mmu.readMemory(addr), 0); break; // SUB M
			case 0x97: instr_sub(a, 0); break; // SUB A

				// SBB
			case 0x98: instr_sub(b, cc.cy); break; // SBB B
			case 0x99: instr_sub(c, cc.cy); break; // SBB C
			case 0x9a: instr_sub(d, cc.cy); break; // SBB D
			case 0x9b: instr_sub(e, cc.cy); break; // SBB E
			case 0x9c: instr_sub(h, cc.cy); break; // SBB H
			case 0x9d: instr_sub(l, cc.cy); break; // SBB L
			case 0x9e: instr_sub(mmu.readMemory(addr), cc.cy); break; // SBB M
			case 0x9f: instr_sub(a, cc.cy); break; // SBB A

				// ANA
			case 0xa0: instr_ana(b); break; // ANA B
			case 0xa1: instr_ana(c); break; // ANA C
			case 0xa2: instr_ana(d); break; // ANA D
			case 0xa3: instr_ana(e); break; // ANA E
			case 0xa4: instr_ana(h); break; // ANA H
			case 0xa5: instr_ana(l); break; // ANA L
			case 0xa6: instr_ana(mmu.readMemory(addr)); break; // ANA M
			case 0xa7: instr_ana(a); break; // ANA A

				// XRA
			case 0xa8: instr_xra(b); break; // XRA B
			case 0xa9: instr_xra(c); break; // XRA C
			case 0xaa: instr_xra(d); break; // XRA D
			case 0xab: instr_xra(e); break; // XRA E
			case 0xac: instr_xra(h); break; // XRA H
			case 0xad: instr_xra(l); break; // XRA L
			case 0xae: instr_xra(mmu.readMemory(addr)); break; // XRA M
			case 0xaf: instr_xra(a); break; // XRA A

				// ORA
			case 0xb0: instr_ora(b); break; // ORA B
			case 0xb1: instr_ora(c); break; // ORA C
			case 0xb2: instr_ora(d); break; // ORA D
			case 0xb3: instr_ora(e); break; // ORA E
			case 0xb4: instr_ora(h); break; // ORA H
			case 0xb5: instr_ora(l); break; // ORA L
			case 0xb6: instr_ora(mmu.readMemory(addr)); break; // ORA M
			case 0xb7: instr_ora(a); break; // ORA A

				// CMP
			case 0xb8: instr_cmp(b); break; // CMP B
			case 0xb9: instr_cmp(c); break; // CMP C
			case 0xba: instr_cmp(d); break; // CMP D
			case 0xbb: instr_cmp(e); break; // CMP E
			case 0xbc: instr_cmp(h); break; // CMP H
			case 0xbd: instr_cmp(l); break; // CMP L
			case 0xbe: instr_cmp(mmu.readMemory(addr)); break; // CMP M
			case 0xbf: instr_cmp(a); break; // CMP A

				// INR
			case 0x04: b = instr_inr(b); break; // INR B
			case 0x0c: c = instr_inr(c); break; // INR C
			case 0x14: d = instr_inr(d); break; // INR D
			case 0x1c: e = instr_inr(e); break; // INR E
			case 0x24: h = instr_inr(h); break; // INR H
			case 0x2c: l = instr_inr(l); break; // INR L
			case 0x34: 
				mmu.writeMemory(addr, instr_inr(mmu.readMemory(addr))); 
				break; // INR M
			case 0x3c: a = instr_inr(a); break; // INR A

				// DCR
			case 0x05: b = instr_dcr(b); break; // DCR B
			case 0x0d: c = instr_dcr(c); break; // DCR C
			case 0x15: d = instr_dcr(d); break; // DCR D
			case 0x1d: e = instr_dcr(e); break; // DCR E
			case 0x25: h = instr_dcr(h); break; // DCR H
			case 0x2d: l = instr_dcr(l); break; // DCR L
			case 0x35: 
				mmu.writeMemory(addr, instr_dcr(mmu.readMemory(addr))); 
				break; // DCR M
			case 0x3d: a = instr_dcr(a); break; // DCR A
			...
			case 0x33: sp = (sp + 1) & 0xffff; break; // INX SP
			...
			case 0x3b: sp = (sp - 1) & 0xffff; break; // DCX SP
			...
			case 0x3f: cc.cy = (cc.cy == 1) ? (byte) 0 : 1; break; // CMC	

				// ALU (IMMEDIATE)
			case 0xc6: instr_add(mmu.readMemory(opcode + 1), 0); pc++; break; // ADI D8		
			case 0xce: instr_add(mmu.readMemory(opcode + 1), cc.cy); pc++; break; // ACI D8	
			case 0xd6: instr_sub(mmu.readMemory(opcode + 1), 0); pc++; break; // SUI D8	
			case 0xde: instr_sub(mmu.readMemory(opcode + 1), cc.cy); pc++; break; // SBI D8
			case 0xe6: instr_ana(mmu.readMemory(opcode + 1)); pc++; break; // ANI D8
			case 0xee: instr_xra(mmu.readMemory(opcode + 1)); pc++; break; // XRI D8
			case 0xf6: instr_ora(mmu.readMemory(opcode + 1)); pc++; break; // ORI D8
			case 0xfe: instr_cmp(mmu.readMemory(opcode + 1)); pc++; break; // CPI D8
			...
			case 0xe9: pc = addr; break; // PCHL

				// RET (conditional)
			case 0xc0: conditional_ret(cc.z == 0); break; // RNZ
			case 0xc8: conditional_ret(cc.z == 1); break; // RZ
			case 0xd0: conditional_ret(cc.cy == 0); break; // RNC
			case 0xd8: conditional_ret(cc.cy == 1); break; // RC
			case 0xe0: conditional_ret(cc.p == 0); break; // RPO
			case 0xe8: conditional_ret(cc.p == 1); break; // RPE	
			case 0xf0: conditional_ret(cc.s == 0); break; // RP
			case 0xf8: conditional_ret(cc.s == 1); break; // RM

				// JMP (conditional)
			case 0xc2: conditional_jmp(opcode, cc.z == 0); break; // JNZ adr
			case 0xca: conditional_jmp(opcode, cc.z == 1); break; // JZ adr
			case 0xd2: conditional_jmp(opcode, cc.cy == 0); break; // JNC adr
			case 0xda: conditional_jmp(opcode, cc.cy == 1); break; // JC adr
			case 0xe2: conditional_jmp(opcode, cc.p == 0); break; // JPO adr
			case 0xea: conditional_jmp(opcode, cc.p == 1); break; // JPE adr
			case 0xf2: conditional_jmp(opcode, cc.s == 0); break; // JP adr
			case 0xfa: conditional_jmp(opcode, cc.s == 1); break; // JM adr

				// CALL (conditional)
			case 0xc4: conditional_call(opcode, cc.z == 0); break; // CNZ adr
			case 0xcc: conditional_call(opcode, cc.z == 1); break; // CZ adr
			case 0xd4: conditional_call(opcode, cc.cy == 0); break; // CNC adr
			case 0xdc: conditional_call(opcode, cc.cy == 1); break; // CC adr
			case 0xe4: conditional_call(opcode, cc.p == 0); break; // CPO adr
			case 0xec: conditional_call(opcode, cc.p == 1); break; // CPE adr
			case 0xf4: conditional_call(opcode, cc.s == 0); break; // CP adr
			case 0xfc: conditional_call(opcode, cc.s == 1); break; // CM adr
			...
			case 0xff: instr_call(0x38); break; // RST 7
				...
				// INTERRUPTS
			case 0xf3: hasInterrupt = 0; break; // DI
			case 0xfb: hasInterrupt = 1; break; // EI

				// I/O
			case 0xd3: pc++; break; // OUT D8
			case 0xdb: pc++; break; // IN D8 (stub) (Load I/O to Accumulator)
	...
	private void push_psw() {
		...
		int psw =
			(cc.s     <<  7)  |   // place sign flag status on pos 7
			...
			(1            <<  1)  |
			(cc.cy)  ;   // place carry flag status on pos 0
